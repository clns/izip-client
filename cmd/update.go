...
func CheckUpdate() {
	...
	shouldCheckForUpdate := !(err == nil && time.Now().Sub(fi.ModTime()) < 10*time.Minute)
	if !shouldCheckForUpdate {
		log.Println("update: skip checking, last check was under 10 minutes ago")
...
func printUpdateAvl(latest string) {
	if latest != Version {
		fmt.Printf("New update available: %s. Run '%s update' to update.\n", latest, NAME)
	}
}
